...
const updateTour = async (req, res) => {
    try {
      ...
      // Fetch the existing tour
      const existingTour = await Tour.findOne({ tourId: req.params.tourId });
      if (!existingTour) {
        return res.status(404).json({ message: "Tour not found" });
      }
      ...
      if (tourName && tourName !== existingTour.tourName) {
        ...
        // Check if the new slug is already in use by another tour
        const slugExists = await Tour.findOne({
          slug,
          tourId: { $ne: req.params.tourId },
        });
        if (slugExists) {
      ...
      const tourCoverFile = req.files?.tourCover?.[0];
      const tourGalleryFiles = req.files?.tourGallery || [];
      ...
      // Update the tour with new fields
